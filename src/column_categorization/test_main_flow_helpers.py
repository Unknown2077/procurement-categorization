...
from column_categorization import cli_common
from column_categorization import cli_interactive
from column_categorization import cli_resolution
...
def test_normalize_mode_maps_legacy_etl_alias() -> None:
    assert cli_interactive._normalize_mode("etl") == "categorize_to_api"
    assert cli_interactive._normalize_mode("to_api") == "to_api"
    assert cli_interactive._normalize_mode("raw_to_api") == "raw_to_api"


def test_parse_bool_text_parses_true_and_false() -> None:
    assert cli_interactive._parse_bool_text("yes", "DO_CATEGORIZE") is True
    assert cli_interactive._parse_bool_text("off", "DO_CATEGORIZE") is False


def test_default_yes_for_do_categorize_prompt_uses_env_and_defaults_false(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("DO_CATEGORIZE", raising=False)
    assert cli_interactive._default_yes_for_do_categorize_prompt() is False
    monkeypatch.setenv("DO_CATEGORIZE", "true")
    assert cli_interactive._default_yes_for_do_categorize_prompt() is True
    monkeypatch.setenv("DO_CATEGORIZE", "0")
    assert cli_interactive._default_yes_for_do_categorize_prompt() is False


def test_describe_api_check_base_url_source_prefers_cli() -> None:
    arguments = argparse.Namespace(api_base_url="https://cli.example")
    assert cli_interactive._describe_api_check_base_url_source(arguments) == "cli (--api-base-url)"


def test_describe_api_check_base_url_source_falls_back_to_env(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("TARGET_API_BASE_URL", "https://api.example")
    arguments = argparse.Namespace(api_base_url=None)
    assert cli_interactive._describe_api_check_base_url_source(arguments) == "env (TARGET_API_BASE_URL)"


def test_describe_categorize_only_target_source_labels() -> None:
    n = _neutral_runtime(source_query="SELECT 1", categorize_columns=("a", "b"))
    assert (
        cli_interactive._describe_categorize_only_target_columns_source(
            argparse.Namespace(target_columns_json='[{"name":"x"}]', categorized_columns=None),
            n,
        )
        == "cli (--target-columns-json)"
    )
    assert (
        cli_interactive._describe_categorize_only_target_columns_source(
            argparse.Namespace(target_columns_json=None, categorized_columns="u, v"),
            _neutral_runtime(source_query="SELECT 1"),
        )
        == "cli (--categorized-columns)"
    )
    assert (
        cli_interactive._describe_categorize_only_target_columns_source(
            argparse.Namespace(target_columns_json=None, categorized_columns=None),
            n,
        )
        == "env (CATEGORIZE_COLUMNS: a, b)"
    )
    assert (
        cli_interactive._describe_categorize_only_target_columns_source(
            argparse.Namespace(target_columns_json=None, categorized_columns=None),
            _neutral_runtime(source_query="SELECT 1", categorize_columns=()),
        )
        == "interactive (will prompt) / pending"
    )


def test_cli_categorize_only_exports_run_mode() -> None:
    assert callable(cli_categorize_only._run_categorization_only_mode)


def test_parse_raw_columns_uses_fallback() -> None:
    output = cli_common._parse_raw_columns(None, fallback_columns=["source_event_id", "raw_value"])
    assert output == ["source_event_id", "raw_value"]


def test_parse_raw_columns_parses_comma_list() -> None:
    output = cli_common._parse_raw_columns(
...
def test_parse_categorized_columns_uses_fallback() -> None:
    output = cli_common._parse_categorized_columns(None, fallback_column="note")
    assert output == ["note"]


def test_parse_categorized_columns_parses_comma_list() -> None:
    output = cli_common._parse_categorized_columns("note, description", fallback_column="ignored")
...
def test_resolve_source_query_uses_cli_value() -> None:
    ...
    output = cli_resolution._resolve_source_query(arguments, neutral)
...
def test_resolve_effective_batch_size_prefers_cli() -> None:
    ...
    assert cli_resolution._resolve_effective_batch_size(arguments, neutral) == 7
...
def test_resolve_effective_batch_size_falls_back_to_neutral() -> None:
    ...
    assert cli_resolution._resolve_effective_batch_size(arguments, neutral) == 12
...
def test_resolve_categorize_column_names_reads_neutral_csv_when_flagged() -> None:
    ...
    output = cli_common._resolve_categorize_column_names(
        arguments,
        neutral,
...
def test_fetch_source_rows_with_retry_lists_relations_and_reprompts() -> None:
    ...
    with pytest.MonkeyPatch.context() as monkeypatch:
        monkeypatch.setattr(cli_common, "list_accessible_relations", lambda _: ["public.demo_table"])
        monkeypatch.setattr(cli_common, "_prompt_text", lambda *_args, **_kwargs: "SELECT id, description FROM public.demo_table")
        rows, columns, resolved_sql = cli_common._fetch_source_rows_with_retry(
...
def test_resolve_llm_runtime_prefers_cli_over_neutral() -> None:
    ...
    neutral = _neutral_runtime(source_query="SELECT 1")
    key, base, model = cli_common._resolve_llm_runtime(arguments, neutral)
    assert key == "cli-key"
...
def test_ensure_required_columns_raises_on_missing_column() -> None:
    with pytest.raises(ValueError, match="SOURCE_QUERY result is missing required columns: note"):
        cli_common._ensure_required_columns(
...
def test_serialize_categorized_labels_returns_json_string() -> None:
    output = cli_common._serialize_categorized_labels(["Jasa Survei", "Jasa Pemetaan"])
...
def test_categorize_batch_with_retries_succeeds_after_transient_failure() -> None:
    ...
    result = cli_common._categorize_batch_with_retries(
...
def test_categorize_batch_with_retries_raises_after_exhausted() -> None:
    ...
    with pytest.raises(RuntimeError, match="Categorization failed"):
        cli_common._categorize_batch_with_retries(
            categorizer,  # type: ignore[arg-type]
            categorized_column="c",
            batch_index=2,
            values_batch=["x"],
